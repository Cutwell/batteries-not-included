...
local function newModel(verts, texture, translation, rotation, scale)
    ...
    self.translation = {translation or {0,0,0}, rotation or {0,0,0}, scale or {1,1,1}}
    ...
    self.position = translation or {0,0,0}

    return self
end

function model:addCollisionModel(m)
    table.insert(self.collisionModels, m)
    return m
end

-- collide against all models in my collision list
-- and return the collision against the closest one
function model:collisionTest(mx,my,mz)
    local bestLength, bx,by,bz, bnx,bny,bnz

    for _,m in ipairs(self.collisionModels) do
        local len, x,y,z, nx,ny,nz = m:capsuleIntersection(
            self.position[1] + mx,
