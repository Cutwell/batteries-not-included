...
function primitives.setRayPosition(x,y,z)
    rayModel.position = {x,y,z}
end

function primitives.setRayCollision(map)
    rayModel:addCollisionModel(map)
end

function primitives.rayCollision(mx,my,mz)
    return rayModel:collisionTest(mx,my,mz)
end

function primitives.ray(x1,y1,z1,x2,y2,z2)
    --local v_x,v_y,v_z = g3d.camera.getLookVector()
    local v_x = (x1+x2)/2 - g3d.camera.position[1]
    local v_y = (y1+y2)/2 - g3d.camera.position[2]
    local v_z = (z1+z2)/2 - g3d.camera.position[3]
    ...
    rayModel.mesh:setVertex(1, x1-n_x, y1-n_y, z1-n_z)
    rayModel.mesh:setVertex(2, x1+n_x, y1+n_y, z1+n_z)
    rayModel.mesh:setVertex(3, x2-n_x, y2-n_y, z2-n_z)
    rayModel.mesh:setVertex(4, x2-n_x, y2-n_y, z2-n_z)
    rayModel.mesh:setVertex(5, x2+n_x, y2+n_y, z2+n_z)
    rayModel.mesh:setVertex(6, x1+n_x, y1+n_y, z1+n_z)
end

local billboardModel = g3d.newModel("assets/board.obj", "assets/tutorial/billboard_1.png")

function primitives.updateBillboardTexture(texture)
    billboardModel.mesh:setTexture(texture)
end
...
local rectangleModel = g3d.newModel("assets/board.obj", "assets/tutorial/billboard_2.png")

local translation = {}
local rotation = {}
local scale = {}
function primitives.rectangle(x,y,z, rx,ry,rz, sx,sy,sz, model)
    local model = model or rectangleModel
    translation[1] = x
    translation[2] = y
    translation[3] = z
    rotation[1] = rx
    ...
    scale[2] = sy
    scale[3] = sz
    model:setTransform(x and translation, rx and rotation, sx and scale)
    model:draw()
end

function primitives.updateRectangleTexture(texture)
    rectangleModel.mesh:setTexture(texture)
end
